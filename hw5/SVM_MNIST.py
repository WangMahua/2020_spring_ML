...
class BinarySVM:
    def __init__(self, kernel='rbf', alg='SMO', gamma=None, degree=None, C=1.0, verbose=False):
        self.eps = 1e-6
        self.alg = alg
    ...
    def fit(self, x, y):
        ...
        sv_indices = filter(lambda i:self.alphas[i] > self.eps, list(range(len(y))))
...
class MultiSVM:
    def __init__(self, kernel='linear', alg='SMO', decision_function='ovr', gamma=None, degree=None, C=1.0):
        ...
        self.kernel = kernel
        self.n_class, self.classifiers = 5, []
...
def main():
	...
	#read file
	X_test = np.array(read_file('data/X_test.csv'))
	X_train = np.array(read_file('data/X_train.csv'))
	Y_test = np.array(read_file('data/Y_test.csv'))
	Y_train = np.array(read_file('data/Y_train.csv'))
	clf = MultiSVM()
	clf.fit(X_test, Y_test)
	target_predict = clf.predict(X_train)
	num = 0
	for i in len(target_predict):
		if target_predict[i] == Y_train[i]:
			num+=1
	print("correct rate: %f ",num/2500)
